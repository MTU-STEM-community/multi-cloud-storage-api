import {
  ...
  export class StorageController {
    ...
    @ApiParam({ name: 'provider', enum: ['google', 'dropbox'], description: 'Cloud provider name' })
    @ApiBody({
      schema: {
        type: 'object',
        properties: {
    ...
    @ApiResponse({
      ...
      schema: {
        ...
        properties: {
          ...
          storageName: { type: 'string', description: 'Name used in cloud storage' }
        }
      }
    })
    ...
    async uploadFile(
      ...
      const result = await this.storageService.uploadFileToProvider(file, provider);
      return {
        url: result.url,
        originalName: file.originalname,
        storageName: result.storageName // This will be added to storage service
      };
    }
    ...
    @ApiParam({ name: 'provider', enum: ['google', 'dropbox'], description: 'Cloud provider name' })
    @ApiParam({ name: 'fileId', description: 'ID or name of the file to download' })
    @ApiQuery({ name: 'originalName', required: false, description: 'Original file name for the download' })
    async downloadFile(
      @Param('provider') provider: string,
      ...
      const fileData = await this.storageService.downloadFileFromProvider(provider, fileId);

      // Extract file type for content-type header
      let contentType = 'application/octet-stream'; // Default

      // Try to determine content type from fileId if it has extension
      if (fileId.includes('.')) {
      ...
      // Use original name for download if provided, otherwise use the cloud storage name
